...
class DeepRM_TC(nn.Module):
    '''
    NARRE: WWW 2018
    '''
    def __init__(self, opt):
        super(DeepRM_TC, self).__init__()
        self.opt = opt
        ...
        # self.context_coatt = Co_Attention(dim=self.opt.id_emb_size, gumbel=False, pooling='avg')

        self.word_coatt = new_Co_Attention(gumbel=False, pooling='avg')
        self.review_coatt = new_Co_Attention(gumbel=False, pooling='avg')
        self.context_coatt = new_Co_Attention(gumbel=False, pooling='avg')
    ...
    def forward(self, datas):
        ...
        context_embedding = torch.stack((user_id_emb, item_id_emb, type_emd, month_emd), 1)

        # --------review_contexte_co-attention --------------------
        review_att, context_att = self.context_coatt(review_embedding, context_embedding)
        context_att_embedding = context_embedding * context_att
        user_id_emb, item_id_emb, type_emd, month_emd = torch.split(context_att_embedding, 1, 1)
