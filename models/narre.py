...
class Net(nn.Module):
    ...
    def forward(self, reviews, ids, ids_list):
        ...
        reviews = reviews.view(-1, r_len, wd)   #[32*5,5,300]

        id_emb = self.id_embedding(ids)   #[32,32]
        u_i_id_emb = self.u_i_id_embedding(ids_list)  #[32,5,32]

        # --------cnn for review--------------------
        fea = F.relu(self.cnn(reviews.unsqueeze(1))).squeeze(3)    # [160,100,3]
        fea = F.max_pool1d(fea, fea.size(2)).squeeze(2)   #[160,100]
        fea = fea.view(-1, r_num, fea.size(1))   #[32,5,100]

        # ------------------linear attention-------------------------------
        rs_mix = F.relu(self.review_linear(fea) + self.id_linear(F.relu(u_i_id_emb)))   #[32,5,32]
        att_score = self.attention_linear(rs_mix)   #[32,5,1]
        att_weight = F.softmax(att_score, 1)  #[32,5,1]
        r_fea = fea * att_weight   #[32,5,100]
        r_fea = r_fea.sum(1)
