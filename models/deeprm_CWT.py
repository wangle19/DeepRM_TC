...
class DeepRM_TC_CWT(nn.Module):
    '''
    NARRE: WWW 2018
    '''

    def __init__(self, opt):
        super(DeepRM_TC_CWT, self).__init__()
        self.opt = opt
        self.num_fea = 3  # ID + Review +context

        # id embedding
        self.user_id_embedding = nn.Embedding(self.opt.user_num, opt.id_emb_size)  # user/item num * 32
        self.item_id_embedding = nn.Embedding(self.opt.item_num, opt.id_emb_size)  # user/item num * 32
        self.type_embedding = nn.Embedding(self.opt.type_num, opt.id_emb_size)
        self.month_embedding = nn.Embedding(self.opt.month_num, opt.id_emb_size)
        ...
        self.review_coatt = new_Co_Attention(gumbel=False, pooling='avg')

        self.relu = nn.ReLU()
        self.user_fc_layer = nn.Linear(self.opt.id_emb_size, self.opt.id_emb_size)
        self.item_fc_layer = nn.Linear(self.opt.id_emb_size, self.opt.id_emb_size)
    ...
    def forward(self, datas):
        ...
        user_reviews = self.fc_g1(user_reviews)
        item_reviews = self.fc_g2(item_reviews)

        user_word_embedding = user_reviews.view(batch, -1, self.opt.id_emb_size)
        item_word_embedding = item_reviews.view(batch, -1, self.opt.id_emb_size)

        word_embedding = torch.concat((user_word_embedding, item_word_embedding), 1)

        # --------context_word_co-attention --------------------
